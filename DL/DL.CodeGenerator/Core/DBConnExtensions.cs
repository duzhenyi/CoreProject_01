...
{
    public static class DBConnExtensions
    {
        /// <summary>
        /// 获取完整数据库信息包含表和列的信息
        /// </summary>
        ...
        /// <returns></returns>
        public static List<DBTable> GetCurrentDatabaseTableList(this IDbConnection dbConnection, DBType dbType)
        {
            ...
            tables.ForEach(item =>
            {
                ...
                {
                    var csharpType = DBColumnTypes.DBColumnDataTypes.FirstOrDefault(
                        t => t.DatabaseType == dbType && t.ColumnTypes.Split(',')
                                                         .Any(p => p.Trim()
                                                         .Equals(x.ColumnType, StringComparison.OrdinalIgnoreCase)))?
                                                         .CSharpType;
        ...
        private static List<DBTable> GetCurrentDatabaseAllTables(this IDbConnection dbConnection, DBType dbType)
        {
        ...
        /// <returns></returns>
        private static List<DBTableColumn> GetColumnsByTableName(this IDbConnection dbConnection, DBType dbType, string tableName)
        {
            if (dbConnection == null) throw new ArgumentNullException(nameof(dbConnection));
            if (dbConnection.State == ConnectionState.Closed) dbConnection.Open();
...
FROM      sys.syscolumns AS a LEFT OUTER JOIN
        ...
        private static string strGetAllColumnsSql(this IDbConnection dbConnection, DBType dbType, string tableName)
        {
